...
from live_danmu import Danmuku
...
class Ui_MainWindow(object):
    ...
    def start_monitor(self):
        ...
        # try:
        #     self.danmuku.start_monitor(stats_report_func, room_id, paizi, keyword)
        # except Exception as e:
        #     QMessageBox.critical(self.main_window, "连接Bilibili直播服务时出现错误", format_exc())

        # Create a new thread and run the danmuku worker
        self.thread = QThread()
        # Create the danmuku worker and allocate to thread
        self.danmuku = Danmuku(room_id, paizi, keyword)
        self.danmuku.moveToThread(self.thread)
        # Setup progress reporting signals
        self.danmuku.new_danmu.connect(self.update_lcd)
        self.danmuku.viewer_report.connect(self.lottery_process)
        # Setup start and finish signals
        self.thread.started.connect(self.danmuku.start_monitor)  # Run main logic method when thread starts
        self.danmuku.finished.connect(self.thread.quit)  # Quit the thread when job done
        self.danmuku.finished.connect(self.danmuku.deleteLater)  # plan to delete itself when job done
        self.thread.finished.connect(self.thread.deleteLater)  # Delete the thread when it is finished
